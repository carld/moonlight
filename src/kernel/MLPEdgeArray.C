...
boolean MLPEdgeArray::_serialWrite( XDRFile* file )
{
  if( file == 0 )
    return false;
...
// return false if error
boolean MLPEdgeArray::_serialRead( XDRFile* file )
{
  if( file == 0 )
    return false;
...
int MLPEdgeArray::_serialSize() const
{
  int s= 0;
...
MLProperty* MLPEdgeArray::clone() const
{
  MLPEdgeArray* a= new MLPEdgeArray(0);
  a->array= array;
  return a;
}
...
MLString MLPEdgeArray::elementToString( const MLEdge& t ) const
{
  return MLString::printf( "MLEdge@%p", &t );
}
