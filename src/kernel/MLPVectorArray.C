...
boolean MLPVectorArray::_serialWrite( XDRFile* file )
{
  if( file == 0 )
    return false;
...
// return false if error
boolean MLPVectorArray::_serialRead( XDRFile* file )
{
  if( file == 0 )
    return false;
...
int MLPVectorArray::_serialSize() const
{
...
MLProperty* MLPVectorArray::clone() const
{
  MLPVectorArray* a= new MLPVectorArray(0);
  a->array= array;
  return a;
}
...
MLString MLPVectorArray::elementToString( const Vector& v ) const
{
