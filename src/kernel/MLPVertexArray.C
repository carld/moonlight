...
boolean MLPVertexArray::_serialWrite( XDRFile* file )
{
  if( file == 0 )
    return false;
...
int MLPVertexArray::_serialSize() const
{
  int s= 0;
...
MLProperty* MLPVertexArray::clone() const
{
  MLPVertexArray* a= new MLPVertexArray(0);
  a->array= array;
  return a;
}
...
MLString MLPVertexArray::elementToString( const MLVertex& v ) const
{
  return MLString::printf( "MLVertex@%p", &v );
}
