...
void MLPPoint::setKey( int _frame )
{
  // anim != 0
...
boolean MLPPoint::setFrame( int _frame )
{
  if( anim == 0 )
...
boolean MLPPoint::_serialWrite( XDRFile* file )
{
  if( file == 0 )
    return false;
...
// return false if error
boolean MLPPoint::_serialRead( XDRFile* file )
{
  if( file == 0 )
    return false;
...
int MLPPoint::_serialSize() const
{
...
boolean MLPPoint::copy( MLProperty* p ) const
{
...
MLProperty* MLPPoint::clone() const
{
  MLPPoint* p= new MLPPoint(0);
  p->v= v;
  return p;
}
...
MLString MLPPoint::toString() const
{
