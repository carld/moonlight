...
/*
  ...
  boolean MLSceneIO::read_1_3( MLScene* scene, MLRFileBlock& );
...
boolean MLSceneIO::read_1_3( MLScene* scene, MLRFileBlock& block )
{
  int i;
  ...
  block.go(MLSceneID::SCENE_HIER);
    ...
    for( i= 1; i <= nHier; ++i )
      ...
      if( scene->getObjectByFilename(oname.filename) == 0 )
        {
        MLObject* o= MLObjectIO::read(database,scene,oname.filename,progress);
#ifndef NDEBUG
        if( o == 0 )
          {
          // warning
    ...
    for( i= 1; i <= nPools; ++i )
      ...
      if( pool == 0 )
        ...
        if( pool->getNLights() == 0 ||
            pool->getName() == MLString(MLLightPool::defaultPoolName) )
          newPool= true;
        }
...
//        glighting->change_light_pool( light, pool );
        ...
        if( old_pool != pool )
          {
          boolean save_dirty;
          boolean save_dirtyIndirect;
          if( old_pool == 0 )
            old_pool= scene->getPool(MLLightPool::defaultPoolName);

          if( old_pool != 0 )
            {
            save_dirty= old_pool->dirty;
            save_dirtyIndirect= old_pool->dirtyIndirect;
            old_pool->removeLight(light);
            old_pool->dirty= save_dirty;
            old_pool->dirtyIndirect= save_dirtyIndirect;
            }

          save_dirty= pool->dirty;
          save_dirtyIndirect= pool->dirtyIndirect;
          pool->addLight(light);
          pool->dirty= save_dirty;
          pool->dirtyIndirect= save_dirtyIndirect;
          }
        }
...
printf( "Unexpected error: unknown object '%s'\n", oname.name.get() );
      ...
      progress->setMessage( MLString("Reading object ") + m->getName().name );
      ...
      lighting->dirty= block.getInteger();
      lighting->dirtyFinal= block.getInteger();
      ...
      if( hasIndirectPES )
        ...
        if( valid )
          readArray( block, pes->energy, 0, nPools-1 );
         else
          skipArray( block, Spectrum(0,0,0), 0, nPools-1 );
      ...
      for(;;)
        ...
        assert( pool != 0 );
        // poolLighting will be NULL for DIRECT_POOL pool type

        boolean save_dirty= pool->dirty;
        boolean save_dirtyIndirect= pool->dirtyIndirect;
        boolean save_lighting_dirty= lighting->dirty;
        MLMeshPoolLighting* poolLighting=
                                         lighting->prepareForShooting(pool);
        lighting->dirty= save_lighting_dirty;
        pool->dirty= save_dirty;
        pool->dirtyIndirect= save_dirtyIndirect;

        boolean poolLighting_is_dirty= (block.getInteger() != 0);
        if( poolLighting != 0 )
          poolLighting->dirty= poolLighting_is_dirty;
        ...
        if( poolLighting != 0 )
          ...
          skipArray( block, MLFormFactor(0), 1, size );
        ...
        if( poolLighting != 0 )
          ...
          skipArray( block, Spectrum(0,0,0), 1, size );
