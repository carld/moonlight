...
/*
  ...
  boolean MLSceneIO::read_1_2( MLScene* scene, MLRFileBlock& );
...
boolean MLSceneIO::read_1_2( MLScene* scene, MLRFileBlock& block )
{
  int i;
...
//        glighting->change_light_pool( light, pool );
  ...
  block.go(MLSceneID::SCENE_LIGHTING);
    ...
    for(;;)
      ...
      MLName oname;
      block >> oname;
      MLMesh* m= (MLMesh*) scene->getObject(oname,MLObject::MESH);
      if( m == 0 )
{
