...
MLFHideUnhide::MLFHideUnhide( MLModule* _module ):
  MLBaseFunction(_module)
{}

/////////////////////////////////////////////////////////////////////////////

MLFHideUnhide::~MLFHideUnhide()
{}

/////////////////////////////////////////////////////////////////////////////

boolean MLFHideUnhide::_call()
{
  ...
  if( model->getFlags()->singleSelection )
    model->pushMessage( "Select object to hide/unhide" );
   else
    model->pushMessage( "Select object(s) to hide/unhide" );
  ...
  while( ! exitFunction )
    ...
    cmd->type= ModelCommands::HIDE_UNHIDE;
    cmd->indices= s->objects;

    if( model->getFlags()->singleSelection )
      cmd->indices.truncate(1);

    modelScene->setHighlighted(cmd->indices);
    model->refreshScreen(ModelModule::REFRESH_SELECTION);
