...
//static char *RCS_Id =
//"@(#) $Id: makefont.C,v 1.1 2002/01/21 11:51:16 oxygene Exp $";

/*
$Id: makefont.C,v 1.1 2002/01/21 11:51:16 oxygene Exp $

$Log: makefont.C,v $
Revision 1.1  2002/01/21 11:51:16  oxygene
- first checkin
...
void
glXDumpXFont( Font font, int first, int count, int listbase,
              GLXFontStruct* glxfont )
{
  XMesaContext CC;
  Display *dpy;
  Window win;
  ...
  font_height= fs->ascent + fs->descent;
  ...
  for (i = 0; i < count; i++)
    {
      ...
      if (fs->per_char
          && (c >= int(fs->min_char_or_byte2))
          && (c <= int(fs->max_char_or_byte2)))
      ...
      glNewList (list, GL_COMPILE);
        if ((c >= int(fs->min_char_or_byte2))
            ...
            && (bm_width > 0) && (bm_height > 0))
          {
            ...
            glxfont[i].width= width;
            glxfont[i].height= height;
            glxfont[i].x0= x0;
            glxfont[i].y0= y0;
            glxfont[i].xi= dx;
            glxfont[i].yi= dy;
            int length= bm_width * bm_height;
            glxfont[i].length= length;
            glxfont[i].data= new GLubyte [ length ];
            for( int k= 0; k < length; ++k )
              glxfont[i].data[k]= bm[k];
#ifdef DEBUG
            if (debug_xfonts)
              {
